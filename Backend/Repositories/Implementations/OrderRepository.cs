...
namespace FlowerSellingWebsite.Repositories.Implementations
{
    public class OrderRepository : BaseRepository<Orders>, IOrderRepository
    {
        ...
        private IQueryable<Orders> ApplySorting(IQueryable<Orders> query, string sortBy, bool sortDescending)
        {
        ...
        public async Task<OrderDetails?> GetOrderDetailsByPublicIdAsync(Guid publicId)
        {
            return await _context.OrderDetails
                .Include(od => od.Product)
                .Include(od => od.FlowerBatch)
        ...
        public async Task<bool> AddOrderDetailsAsync(OrderDetails orderDetail)
        {
            ...
            if (order != null)
            {
                order.EstimatedTotalAmount += orderDetail.LineTotal;
                order.UpdatedAt = DateTime.UtcNow;
                _context.Orders.Update(order);
            }
        ...
        public async Task<bool> UpdateOrderDetailsAsync(OrderDetails orderDetail)
        {
            // Get the original order detail to calculate the difference in amount
            var originalOrderDetails = await _context.OrderDetails.AsNoTracking()
                .FirstOrDefaultAsync(od => od.Id == orderDetail.Id);
            
            if (originalOrderDetails == null || originalOrderDetails.IsDeleted)
                return false;
            ...
            if (order != null)
            {
                ...
                order.EstimatedTotalAmount = order.EstimatedTotalAmount - originalOrderDetails.LineTotal + orderDetail.LineTotal;
                
                // If final amounts are set, update those too
                if (orderDetail.FinalAmount.HasValue && originalOrderDetails.FinalAmount.HasValue && order.FinalTotalAmount.HasValue)
                {
                    order.FinalTotalAmount = order.FinalTotalAmount.Value - originalOrderDetails.FinalAmount.Value + orderDetail.FinalAmount.Value;
                }
        ...
        public async Task<bool> RemoveOrderDetailsAsync(int orderDetailId)
        {
