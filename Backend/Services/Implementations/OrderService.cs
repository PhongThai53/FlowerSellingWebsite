...
namespace FlowerSellingWebsite.Services.Implementations
{
    public class OrderService : BaseService<Orders>, IOrderService
    {
        ...
        {
            var orderDetail = await _orderRepository.GetOrderDetailsAsync(id);
            if (orderDetail == null)
        ...
        {
            var orderDetail = await _orderRepository.GetOrderDetailsByPublicIdAsync(publicId);
            if (orderDetail == null)
        ...
        public async Task<OrderDTO> CreateOrderAsync(CreateOrderDTO createOrderDTO, int userId)
        {
            ...
            foreach (var detailDTO in createOrderDTO.OrderDetails)
            {
                ...
                {
                    ProductId = detailDTO.ProductId ?? 0,
        ...
        public async Task<OrderDetailDTO> AddOrderDetailAsync(int orderId, CreateOrderDetailDTO createOrderDetailDTO)
        {
            ...
            var orderDetail = _mapper.Map<OrderDetails>(createOrderDetailDTO);
            orderDetail.OrderId = orderId;
            orderDetail.PublicId = Guid.NewGuid();
            orderDetail.CreatedAt = DateTime.UtcNow;
            orderDetail.Discount = 0;
            orderDetail.LineTotal = orderDetail.Quantity * orderDetail.UnitPrice;

            var success = await _orderRepository.AddOrderDetailsAsync(orderDetail);
            if (!success)
                throw new Exception("Failed to add order detail.");

            var savedOrderDetail = await _orderRepository.GetOrderDetailsAsync(orderDetail.Id);
        ...
        {
            var orderDetail = await _orderRepository.GetOrderDetailsAsync(orderDetailId);
            if (orderDetail == null)
                return false;

            // Update only the provided fields
            if (updateOrderDetailDTO.Quantity.HasValue)
            {
                orderDetail.Quantity = updateOrderDetailDTO.Quantity.Value;
            ...
            return await _orderRepository.UpdateOrderDetailsAsync(orderDetail);
        }

        public async Task<bool> DeleteOrderDetailAsync(int orderDetailId)
        {
            return await _orderRepository.RemoveOrderDetailsAsync(orderDetailId);
        }
