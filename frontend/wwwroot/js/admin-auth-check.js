...
// Sử dụng cho tất cả các trang admin
...
class AdminAuthChecker {
  ...
  checkWithAuthManager() {
    ...
    if (!window.authManager.isAuthenticated()) {
      this.redirectToLogin('Bạn cần đăng nhập để truy cập trang admin!');
      return;
    }
    ...
    if (currentUser && currentUser.roleName !== 'Admin') {
      this.redirectToHomepage('Bạn không có quyền truy cập trang admin!');
      return;
    }
    
    console.log('Admin access granted for user:', currentUser.fullName);
  }
  ...
  checkWithLocalStorage() {
    ...
    try {
      ...
      console.log('Admin access granted for user:', user.fullName);
    } catch (error) {
